...
countries = movies_df["country"].fillna("Unknown").unique()
countriesStr = ",".join(countries)
countriesStr = countriesStr.replace(", ",",")
country_set = set(countriesStr.split(","))
...
countries_release_count = movies_df.loc[movies_df['release_year'] == year, ["country"]].value_counts()
top_10_countries = countries_release_count.head(10)
...
movies_df["Movie_minutes"] = movies_df.loc[movies_df["type"] == "Movie", "duration"].apply(lambda x: int(x.strip(" min")))
movies = movies_df.loc[movies_df["type"] == "Movie", :]
movies_avg_duration_per_year = movies.groupby(by="release_year")["Movie_minutes"].mean()
