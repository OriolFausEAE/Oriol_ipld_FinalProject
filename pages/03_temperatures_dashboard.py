...
temps_df["AvgTemperatureCelsius"] = (temps_df["AvgTemperatureFahrenheit"] - 32)/(9/5)       # uncomment this line to complete it
...
if unique_countries_list is not None and len(selected_cities) > 0:
    ...
    for city in selected_cities:
        city_df = temps_df.loc[temps_df["City"]==city] # get a dataframe with the rows of the selected city
    ...
    plt.title("Average temperature of selected cities")
    plt.xlabel("Time")
    ...
    for city in selected_cities:
        ...
        city_df_period = city_df.loc[(city_df["Date"] >= start_date) & (city_df["Date"] <= end_date)]
        plt.hist(city_df_period["AvgTemperatureCelsius"], label=city, bins=20)
